...
def _canonical_shared_relative(href: str) -> PurePosixPath | None:
    ...
    if any(part not in ("..", ".") for part in parts[:shared_idx]):
        return None
...
def resolve_asset(
    ...
    *,
    ...
    if is_remote_url(href):
        return None
    effective_shared = _normalize_shared_root(shared_root).resolve()
    allowed_roots = (html_path.parent.resolve(), SHARED.resolve(), effective_shared)

    def _checked(resolved: Path | None) -> Path | None:
        if resolved is None:
            return None
        if not any(resolved == root or resolved.is_relative_to(root) for root in allowed_roots):
            raise ValueError(
                f"Local asset resolves outside allowed roots (deck directory or shared assets): "
                f"{href!r} -> {resolved}"
            )
        return resolved
    ...
    if shared_root is not None and "shared" in href_path.parts:
        ...
        if (
            ...
            return _checked(themes_css.resolve())
        return _checked(_resolve_shared_asset(shared_root, canonical_relative))
    ...
    if "shared" in parts:
        ...
        return _checked((effective_shared / relative).resolve())
    return _checked(candidate)

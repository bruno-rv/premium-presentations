...
class LanSyncServerTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.httpd = lan_sync_server.serve(self.tmpdir.name, 0, host="127.0.0.1")
    ...
    def test_at4_post_then_get_round_trip_under_2s(self) -> None:
        ...
        conn.request("POST", "/slide", body=body, headers={"Content-Type": "application/json"})
    ...
    def test_malformed_post_body_returns_400(self) -> None:
        conn = self._conn()
        conn.request("POST", "/slide", body=b"not json", headers={"Content-Type": "application/json"})
        resp = conn.getresponse()
        resp.read()
        conn.close()
    ...
    def test_missing_content_length_returns_400(self) -> None:
        ...
        try:
            sock.sendall(b"POST /slide HTTP/1.1\r\nHost: 127.0.0.1\r\n\r\n")
    ...
    def test_negative_content_length_returns_400(self) -> None:
        conn = self._conn()
        conn.request("POST", "/slide", body=b"{}", headers={"Content-Length": "-5"})
        resp = conn.getresponse()
        resp.read()
        conn.close()
        self.assertEqual(resp.status, 400)
    ...
    def test_non_integer_content_length_returns_400(self) -> None:
        conn = self._conn()
        conn.request("POST", "/slide", body=b"{}", headers={"Content-Length": "abc"})
        resp = conn.getresponse()
        resp.read()
        conn.close()
        self.assertEqual(resp.status, 400)
    ...
    def test_current_unchanged_after_rejected_posts(self) -> None:
        ...
        for bad_body in (b"not json", json.dumps({"id": 999}).encode(), json.dumps(["x"]).encode()):
            conn = self._conn()
            conn.request(
                "POST", "/slide", body=bad_body, headers={"Content-Type": "application/json"}
            )
            resp = conn.getresponse()
            resp.read()
            conn.close()
